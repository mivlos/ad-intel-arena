...
export async function GET(
  ...
  try {
    ...
    // Try sim key first, then live key — we don't know which mode was used
    const keys = [
    ...
    for (const apiKey of keys) {
      const response = await fetch(`${BASE_URL}/api/ad-scores/run/${uuid}`, {
    ...
    console.error('Ad scores run status error:', err);
    return NextResponse.json(
